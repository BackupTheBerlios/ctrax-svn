...
class FixBG:
    ...
    def CheckSave(self):
        ...
        if not self.issaved:
            ...
            msgtxt = 'Some changes to background nodel may not have been saved. Save now?'
    ...
    def RemovePolygon(self):
        wx.BeginBusyCursor()
        wx.Yield()
        isin = self.undo_fix_hole(self.bgcenter0,self.bgcenter,self.polygons[-1])
        self.undo_fix_hole(self.bgdev0,self.bgdev,self.polygons[-1],isin=isin)
        self.issaved = False
        wx.EndBusyCursor()
